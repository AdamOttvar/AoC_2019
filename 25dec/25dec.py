...
def first_task():
    ...
    while not terminated:
        ...
        if return_code == 2 and output == 10:
            complete_output.append(chr(output))
            val = input(''.join(complete_output))
            val = val + '\n'

            robot_input = []
            for st in val:
                robot_input.append(ord(st))

            ASCII.complete_input = robot_input
            ASCII.input_index = 0
            complete_output = []
        elif return_code == 2:
            complete_output.append(chr(output))
